...
function AdminSettings({ config={}, user, listings=[], contactProps={}, onSave, showToast=()=>{}, lang="es-CO", adminInfo={} }) {
  ...
  const [missionTitleEn,setMissionTitleEn]=useState(config?.mission_title_en||'');
  const [missionBodyEn,setMissionBodyEn]=useState(config?.mission_body_en||'');
  const [globalMissionLang,setGlobalMissionLang]=useState('es');
  ...
  const saveConfig = () => onSave({slaHours, escalationCcEmails, analyticsEnabled, missionSectionsEs:mission, missionTitleEn, missionBodyEn, defaultDelegatePermissions, tooltipsEs, tooltipsEn});
  ...
  <AdminSection title={`🌊 ${lt(lang,'Misión y reglas de participación')}`} subtitle={lt(lang,'Mantén Español Colombia como base. También puedes editar textos visibles en inglés cuando aplique.')} action={<button className="btn-p" style={{minHeight:36,padding:'6px 14px'}} onClick={saveConfig}>💾 {lt(lang,'Guardar')}</button>} open={openSections.mission} onToggle={()=>toggleSection('mission')}>
    ...
    {globalMissionLang==='en' && <div className="fg2">
      <div className="fg full"><label>Mission title (EN)</label><textarea className="admin-textarea" rows={2} value={missionTitleEn} onChange={e=>setMissionTitleEn(e.target.value)}/></div>
      <div className="fg full"><label>Mission body (EN)</label><textarea rows={4} value={missionBodyEn} onChange={e=>setMissionBodyEn(e.target.value)}/></div>
    </div>}
  </AdminSection>
