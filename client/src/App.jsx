...
const Tip = ({ text }) => text ? <span className="tip" tabIndex="0" title={text} data-tip={text} aria-label={text}>ⓘ</span> : null;
...
function UserContact({ name='', email='', whatsapp='', apartments=[], directory, uid='', showToast=()=>{}, onEmail=()=>{}, lang='es-CO', children }) {
  ...
  return <span className="contact-hover-wrap">
    <button type="button" className="contact-name-btn" onClick={(e)=>e.preventDefault()}>{children || finalName}</button>
    <span className="contact-card" onClick={e=>e.stopPropagation()}>
      <strong>{finalName}</strong>
      {aptList.length>0 && <span>🏠 {aptList.join(', ')}</span>}
      {finalEmail && <span className="contact-line">✉️ {finalEmail}<button type="button" onClick={()=>copyText(finalEmail, showToast, lang)}>📋</button><button type="button" onClick={()=>onEmail({ name:finalName, email:finalEmail, whatsapp:finalWhatsapp, apartments:aptList })}>{lang==='en'?'Email':'Email'}</button></span>}
      {finalWhatsapp && <span className="contact-line">📲 {finalWhatsapp}<button type="button" onClick={()=>copyText(finalWhatsapp, showToast, lang)}>📋</button>{waDigits && <a href={`https://wa.me/${waDigits}`} target="_blank" rel="noreferrer">WhatsApp</a>}</span>}
      {!finalEmail && !finalWhatsapp && <span>{lang === 'en' ? 'No contact information available' : 'Sin información de contacto'}</span>}
    </span>
  </span>;
}
...
function AdminSettings({ config={}, user, listings=[], contactProps={}, onSave, showToast=()=>{}, lang="es-CO" }) {
  ...
  const saveUserPermissions = async (u) => {
    try { await api.post('/api/admin/delegate', { actorUid:user.uid, actorEmail:user.email, uid:u.uid, email:u.email, name:u.name, role:u.role || 'user', permissions:u.permissions || DEFAULT_DELEGATE_PERMISSIONS }); showToast('✅ ' + lt(lang,'Permisos guardados')); await loadUsers(); }
    catch(e) { showToast((e.message || String(e)), true); }
  };
  ...
  return <div className="fade"><div className="ph"><div><h1 className="ptitle">⚙️ {lt(lang,'Configuración global')}</h1><p className="psub">{lt(lang,'Solo administradores globales · SLA, copias, misión y plantillas de email')}</p></div></div>
    ...
    <div className="card" style={{marginBottom:18}}><div className="card-hdr"><div><div className="card-title">👥 {lt(lang,'Roles y permisos de usuarios')}</div><div className="psub">{lt(lang,'Define global admins, delegates and standard users. Delegates can receive specific additional permissions.')}</div></div><button className="btn-ghost" onClick={loadUsers}>{usersLoading?lt(lang,'Cargando...'):lt(lang,'Actualizar')}</button></div>{users.length===0?<Empty icon="👥" msg={lt(lang,'No hay usuarios aprobados todavía.')}/>:<div className="table-wrap"><table className="admin-table"><thead><tr><th>{lt(lang,'Usuario')}</th><th>{lt(lang,'Email')}</th><th>{lt(lang,'Rol')}</th><th>{lt(lang,'Permisos del delegado')}</th><th>{lt(lang,'Acción')}</th></tr></thead><tbody>{users.map((u,idx)=><tr key={u.uid || u.email}><td><UserContact name={u.name || lt(lang,'Sin nombre')} email={u.email} uid={u.uid} {...contactProps}/></td><td><span className="copy-inline">{u.email}<button type="button" onClick={()=>copyText(u.email, showToast, lang)}>📋</button><button type="button" onClick={()=>contactProps.onEmail({name:u.name,email:u.email,apartments:(lookupContact(contactProps.directory,{uid:u.uid,email:u.email,name:u.name}).apartments||[])})}>✉️</button></span></td><td><select value={u.role || 'user'} disabled={u.envGlobal} onChange={e=>setUsers(prev=>prev.map((x,i)=>i===idx?{...x,role:e.target.value}:x))}><option value="user">{lt(lang,'Usuario estándar')}</option><option value="delegate_admin">{lt(lang,'Administrador delegado')}</option><option value="global_admin">{lt(lang,'Administrador global')}</option></select>{u.envGlobal&&<div className="help-msg">GLOBAL_ADMIN_EMAILS</div>}</td><td>{(u.role === 'delegate_admin') ? <div style={{display:'flex',flexDirection:'column',gap:6}}>{Object.keys(DEFAULT_DELEGATE_PERMISSIONS).map(k=><label key={k} style={{display:'flex',gap:8,alignItems:'center'}}><input type="checkbox" checked={!!(u.permissions||{})[k]} onChange={e=>setUserPermission(idx,k,e.target.checked)}/>{PERMISSION_LABELS[k]?.[lang==='en'?'en':'es'] || k}</label>)}</div> : <span className="help-msg">{u.role === 'global_admin' ? lt(lang,'Administrador global') : lt(lang,'Usuario estándar')}</span>}</td><td><button className="bsm bs-edit" onClick={()=>saveUserPermissions(u)}>{lt(lang,'Actualizar rol/permisos')}</button></td></tr>)}</tbody></table></div>}</div>
...
.contact-hover-wrap{position:relative;display:inline-flex;align-items:center;max-width:100%;vertical-align:middle}.contact-name-btn{border:0;background:transparent;color:inherit;font:inherit;font-weight:inherit;padding:0;cursor:pointer;text-decoration:underline;text-decoration-style:dotted;text-underline-offset:3px}.contact-card{display:none;position:absolute;left:0;top:calc(100% + 8px);min-width:280px;max-width:360px;background:rgba(255,255,255,.98);border:1px solid rgba(90,105,80,.22);border-radius:14px;box-shadow:0 18px 45px rgba(20,32,26,.22);padding:12px;z-index:500;color:#17313a;font-family:'DM Sans',sans-serif;font-size:.78rem;line-height:1.35}.contact-hover-wrap:hover .contact-card,.contact-hover-wrap:focus-within .contact-card{display:flex;flex-direction:column;gap:7px}.contact-line{display:flex;align-items:center;gap:6px;flex-wrap:wrap;word-break:break-word}.contact-line button,.contact-line a,.copy-inline button{border:1px solid rgba(11,127,140,.18);background:rgba(94,215,198,.12);color:#0b7f8c;border-radius:8px;padding:3px 7px;font-size:.68rem;text-decoration:none;cursor:pointer}.copy-inline{display:inline-flex;align-items:center;gap:5px;flex-wrap:wrap}.copy-inline button{padding:2px 6px}.ir-rep .contact-card,.ar-owner .contact-card{left:auto;right:0}.admin-table .contact-card{left:auto;right:0}.modal-sub{word-break:break-word}
@media(max-width:700px){.contact-card{position:fixed;left:12px!important;right:12px;top:72px;max-width:none;min-width:auto}.contact-hover-wrap:hover .contact-card{display:none}.contact-hover-wrap:focus-within .contact-card{display:flex}}
...
.tip{position:relative;overflow:visible!important;}
.tip:hover::after,.tip:focus::after{content:attr(data-tip);position:absolute;left:50%;top:130%;transform:translateX(-50%);width:max-content;max-width:320px;background:#17313a;color:white;border-radius:12px;padding:10px 12px;font-size:.78rem;font-weight:700;line-height:1.35;text-align:left;box-shadow:0 14px 34px rgba(0,0,0,.26);z-index:1000000;white-space:normal;pointer-events:none;}
.tip:hover::before,.tip:focus::before{content:'';position:absolute;left:50%;top:104%;transform:translateX(-50%);border:7px solid transparent;border-bottom-color:#17313a;z-index:1000001;}
...
@media (pointer:coarse){.tip:hover::after{display:none!important}.tip:focus::after,.tip:active::after{display:block!important}.wf-step:hover .wf-tip{display:none!important}.wf-step:focus .wf-tip,.wf-step:active .wf-tip{display:block!important}}
