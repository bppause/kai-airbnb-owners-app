...
app.patch('/api/incidents/:id/resolve', async (req, res) => {
  ...
  const { data: existing, error: findError } = await supabase.from('incidents').select('*, listings(owner_uid)').eq('id', req.params.id).single();
  if (findError || !existing) return res.status(404).json({ error: 'Not found' });
